...
def get_daily_progress_report_data(report_date: Optional[date] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        structure_types = [dict(row) for row in db.execute(
            text("""
                SELECT COALESCE(ea.tipo_estructura_apoyo, 'Sin dato') AS label,
                       COUNT(DISTINCT ea.id)::int AS total
                FROM operational.entidades_apoyo ea
                WHERE ea.estado_revision <> 'descartada'
                GROUP BY COALESCE(ea.tipo_estructura_apoyo, 'Sin dato')
                ORDER BY total DESC, label ASC
            """)
        ).mappings().all()]
...
def build_daily_progress_report_pdf(data: Dict[str, Any]) -> bytes:
    ...
    def line_chart(title: str, rows: List[Dict[str, Any]]) -> None:
        ...
        for x, point_y, value, _ in points:
            ...
            commands.append(f"0.063 0.165 0.263 rg BT /F2 7 Tf {x - 3:.2f} {point_y + 7:.2f} Td ({_pdf_escape(value)}) Tj ET")
