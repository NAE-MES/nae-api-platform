 ...
 * 1. Se mantiene el orden actual: 0.1 continúa siendo "Entidad a la que pertenece"
 *    y 1.1 continúa siendo "Provincia".
 * 2. Provincia y municipio se implementan mediante secciones, para impedir combinaciones incorrectas.
 * 3. La pregunta 1.3 se mantiene.
...
function crearFormularioMapeoNAE() {
  ...
  form.setDescription(
    ...
    'IMPORTANTE: siempre que sea posible, escriba las respuestas en MAYÚSCULAS y sin acentos, para evitar duplicidades. ' +
    'Esta indicación no se aplica a correos electrónicos, sitios web, enlaces ni usuarios de redes sociales.'
  );
  ...
  form.addSectionHeaderItem()
    ...
    .setHelpText(
      ...
      'Complete un registro por cada espacio que desee informar. Todo este bloque es opcional.'
    );

  for (let i = 1; i <= 5; i++) {
    form.addSectionHeaderItem().setTitle('3.2 Espacio ' + i + ' (opcional)');
    form.addTextItem().setTitle('3.2.' + i + '.1 Espacio').setRequired(false);
    form.addTextItem().setTitle('3.2.' + i + '.2 Dirección / lugar').setRequired(false);
    form.addTextItem().setTitle('3.2.' + i + '.3 Aforo aprox.').setRequired(false);
    form.addTextItem().setTitle('3.2.' + i + '.4 Conectividad / tipo').setRequired(false);
    form.addTextItem().setTitle('3.2.' + i + '.5 Energía alternativa').setRequired(false);
    form.addTextItem().setTitle('3.2.' + i + '.6 Aire acondicionado').setRequired(false);
    form.addTextItem().setTitle('3.2.' + i + '.7 Uso posible').setRequired(false);
  }
